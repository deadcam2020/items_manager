...
import { useDeleteFromCart } from '../hooks/products.mutatios';

const ShoppingCart = () => {

  const { user } = useAuthStore()
  // const { cartProducts, loading, fetchGetCartProducts, deleteProductFromCart } = useProductStore()
    ...
    <>
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white shadow-[0_-4px_10px_rgba(0,0,0,0.1)] p-4 border-t">
        <div className="flex justify-between max-w-xl mx-auto flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          {/* todo           */}
          <button className="md:w-auto bg-red-500 text-white px-5 rounded-lg text-lg font-semibold hover:bg-red-700 transition-all cursor-pointer">
            Eliminar carito
          </button>
        </div>
      </div>
